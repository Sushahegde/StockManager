...
define(['../accUtils', 'knockout'],
  function (accUtils, ko) {
    function CustomerViewModel() {
      ...
      self.customerId = ko.observable(2); // Replace with actual logged-in ID
      ...
      self.loadPortfolio = () => {
        ...
        fetch(url)
          .then(res => res.json())
          .then(data => {
            const stockArray = Object.entries(data).map(([stockId, quantity]) => ({
          ...
          .catch(err => alert('Error loading portfolio: ' + err));
      };
      ...
      self.totalHoldings = ko.computed(() =>
        self.stocks().reduce((sum, stock) => sum + stock.quantity * stock.price, 0)
      );
      ...
      self.profitColor = ko.computed(() =>
        self.profitPercent() >= 0 ? 'green' : 'red'
      );

      self.promptSellStock = (stock) => {
        self.selectedStock(stock);
        console.log("selected stock is ",self.selectedStock);
        self.sellQuantity('');
      ...
      // self.selectedTab = ko.observable('transaction-history');

  ...
  function (accUtils, ko) {
    function AboutViewModel(params) {
      ...
      self.fetchCustomer = async function () {
        const url = `http://localhost:8085/customer/email/${encodeURIComponent(self.loginEmail())}`;
    ...
    self.handleLogin = () => {
      ...
      fetch('http://localhost:8085/customer/authentication', {
        method: 'POST',
        headers: {
        ...
        .then(response => {
          ...
          return response.text(); // Backend returns "true" or "false" as plain text
        })
        .then(result => {
          console.log("hehe2");
          console.log(result);
          ...
          if (result === "true") {
            ...
            window.location.reload(); // Reloads so appController reads new role and rebuilds tabs
